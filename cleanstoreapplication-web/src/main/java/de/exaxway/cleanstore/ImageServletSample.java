...
public class ImageServletSample extends HttpServlet {
    
    private static final long serialVersionUID = 6439315738094263474L;

    @EJB
    PhotoCamView productBean;
    private static final Logger logger = Logger.getLogger(ImageServletSample.class.getCanonicalName());
    // Constants 
...
//            response.setHeader("Content-Length", String.valueOf(p.getImgSrc().length));
            response.setHeader("Content-Length", String.valueOf(getImgSrc().length));
...
//                byteInputStream = new ByteArrayInputStream(p.getImgSrc());
                byteInputStream = new ByteArrayInputStream(getImgSrc());
   ...
   //     }
    ...
    private byte[] getImgSrc() throws IOException {
